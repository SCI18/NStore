(function () {
  ...
  function buildOrderPayload() {
    ...
    return {
      order_id: `NS-${Date.now().toString().slice(-6)}`,
      orders,
      cost: { subtotal: subtotal(cart).toFixed(2) },
  ...
  function notifySale() {
    ...
    if (!ejs.serviceId || !ejs.templateId || !ejs.publicKey) return; // feature not configured — skip silently
    if (typeof window.emailjs === "undefined") return; // SDK didn't load — skip silently
    if (getCart().length === 0) return;

    try {
      const payload = buildOrderPayload();
      window.emailjs.send(ejs.serviceId, ejs.templateId, payload).catch(() => {});
    } catch (err) {
      // Sale tracking is a nice-to-have — never let it interrupt the actual checkout.
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const ejs = (window.STORE_CONFIG && window.STORE_CONFIG.emailjs) || {};
    if (ejs.publicKey && typeof window.emailjs !== "undefined") {
      try {
        window.emailjs.init(ejs.publicKey);
      } catch (err) {
        // Sale tracking is a nice-to-have — never let it block the rest of the page.
      }
    }
    updateBadge(getCart());
    renderCart();
